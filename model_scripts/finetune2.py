...
# Theta_L1 = [v for v in tf.trainable_variables() if '1' in v.name]
# Theta_L2 = [v for v in tf.trainable_variables() if '2' in v.name]
# Theta_L3 = [v for v in tf.trainable_variables() if '3' in v.name]
# Theta_L4 = [v for v in tf.trainable_variables() if '4' in v.name]
# Theta_L5 = [v for v in tf.trainable_variables() if '5' in v.name]
# Theta_L6 = [v for v in tf.trainable_variables() if '6' in v.name]
# Theta_L7 = [v for v in tf.trainable_variables() if '7' in v.name]
...
# saver = tf.train.Saver(tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope="save_scope"))
...
# H__2 =  tf.concat([h_pool_flat3 , h_pool_flat2 ],1)
# H__expand2 =tf.expand_dims(H__2,-1)
# H_2_max = tf.layers.max_pooling1d(   H__expand2, H__expand2.get_shape().as_list()[1] , 1 )
# H_2_avg = tf.layers.average_pooling1d( H__expand2, H__expand2.get_shape().as_list()[1] , 1  )
# H_2_avg = tf.reshape(H_2_avg , [-1 , 1])

H_class1 = tf.concat([D_logit_real , output_true , H_1_avg ] ,1)
# H_class2 = tf.concat([D_logit_real , output_true , H_1_avg ] ,1)
...
with tf.variable_scope('optimizers_loss'):
    ...
    if model_select == 1 or model_select == 2 :
        ...
        if confirm == 0 :
            ...
            penalty  = tf.where(condition1, 100 * (loss_topic1 ), loss_topic1)
    ...
    # output_z = tf.placeholder(tf.float32, [None, 1], name="input_x2")  # Z
    if sparse == 1:
        output_z = tf.placeholder(tf.int32, [None], name="input_x2")  # Z
        ...
        # losif sparse == 1:
        # ses = tf.nn.softmax_cross_entropy_with_logits(labels = output_z, logits = L2_output)
    ...
    train_op1 = tf.contrib.layers.optimize_loss(loss_read_total, global_step=global_step,learning_rate=l_rate, optimizer=opti1,clip_gradients=float(clip_gradients) , variables= theta_read)
    train_op2 = tf.contrib.layers.optimize_loss(Loss_total, global_step=global_step,learning_rate=l_rate, optimizer=opti2,clip_gradients=float(clip_gradients), variables= theta_topic )
...
if model_select == 1 or model_select == 2 :
    for epoch in range(0,num_epochs):
            ...
            for start, end in zip(range(0, number_of_training_data, batch_size),range(batch_size, number_of_training_data, batch_size)):
                ...
                feed_dict3 = {input_x2: trainX[start:end],dropout_keep_prob: 0.5 , input_x1: dummy_input1 , input_x3: dummy_input2 ,\
                    prediction_label1: Tens  }    
                curr_loss1,_ =sess.run([L_read,train_op1 ],feed_dict2)
                curr_loss1_temp,_=sess.run([L_read,train_op1 ],feed_dict3)     
                ...
                feed_dict4 = {input_x2: trainX[start:end],dropout_keep_prob: 0.5 , input_x1: dummy_input2 , input_x3: dummy_input1 ,\
                    prediction_label1: Tens , output_z: trainZ[start:end]  } 
                curr_loss3 , curr_loss2,_ =sess.run([loss_topic ,losses ,train_op2  ],feed_dict4)
                curr_loss3_temp, curr_loss2_temp,_ =sess.run([loss_topic ,losses ,train_op2  ],feed_dict1)   
    ...
    for epoch in range(0,num_epochs):
            ...
            for start, end in zip(range(0, number_of_training_data, batch_size),range(batch_size, number_of_training_data, batch_size)):
                ...
                if counter %50==0:
                    print("Epoch %d\t"%(epoch), "read1:" , curr_loss1 , "\tTOPIC:" , curr_loss3  ,"\tCLASSIFIER:" , np.sum(curr_loss2))
                    # print("read1:" , curr_loss1)
                    # print("TOPIC:" , curr_loss3)
                    # print("CLASSIFIER:" , np.sum(curr_loss2))
            sess.run([epoch_increment ])
...
folder_name  = "complete_fine_tuned_%d" %(model_select)
