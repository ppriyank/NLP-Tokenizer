...
# L_read= 100 * (Read_G_loss+ Read_D_loss) 
...
loss_val2 =  L2_reg + tf.reduce_sum(L_read) 
...
# 1 tmux  without sharing 3 tmux with sharing
...
for epoch in range(curr_epoch,num_epochs):
        ...
        for start, end in zip(range(0, number_of_training_data, batch_size),range(batch_size, number_of_training_data, batch_size)):
            ...
            curr_loss1,lr,_=sess.run([loss_val,learning_rate,train_op],feed_dict1)
            curr_loss2,lr,_=sess.run([loss_val,learning_rate,train_op],feed_dict3)
            loss,counter=loss+curr_loss1+curr_loss2,counter+1
            if counter %50==0:
                print("Epoch %d\t , Loss =%f " %(epoch , curr_loss1+curr_loss2))
        sess.run(epoch_increment)
...
for epoch in range(curr_epoch,num_epochs):
        ...
        for start, end in zip(range(0, number_of_training_data, batch_size),range(batch_size, number_of_training_data, batch_size)):
            ...
            dummy_input1 =trainY[ram]
            feed_dict1 = {input_x2: trainX[start:end],dropout_keep_prob: 0.5 , input_x1: trainY[start:end] , input_x3: dummy_input1, \
            printrediction_label1:ones ,prediction_label2:ones } 
            feed_dict2 = {input_x2: trainX[start:end],dropout_keep_prob: 0.5 , input_x1: trainY[start:end] , input_x3: dummy_input2 , \
            prediction_label1: ones ,prediction_label2: zeros }
            feed_dict3 = {input_x2: trainX[start:end],dropout_keep_prob: 0.5 , input_x1: dummy_input2 , input_x3: dummy_input1, \
            prediction_label1:zeros ,prediction_label2:ones } 
            curr_loss1,_,_=sess.run([loss_val,train_op1 , train_op2 ],feed_dict1)
            curr_loss2,_=sess.run([loss_val,train_op1 ],feed_dict2)
            curr_loss3,lr,_=sess.run([loss_val,learning_rate,train_op2 , train_op1  ],feed_dict3)
...
saver.save(sess, "noshare_no_cross/model.ckpt")



saver = tf.train.Saver()
saver.save(sess, "noshare_overall/model.ckpt")
...
def compute(d):
    ...
    for start, end in zip(range(0, number_examples, batch_size),range(batch_size, number_examples, batch_size)):
                ...
                pred1,pred2,pred3,pred4,read_input , read_incoerent =sess.run([D_real,D_logit_real,D_fake , D_logit_fake, read_real, read_fake ],feed_dict1)
                pred5,pred6,pred7,pred8 , read_input , read_garbage =sess.run([D_real,D_logit_real,D_fake , D_logit_fake , read_real, read_fake],feed_dict2)
                if d == 1:
...
Scores  = [S_G , S_GA , S_IN, read_input__, read_garbage__, read_incoerent__ ]
...
[C,D,E ] = plot(Scores[0:3].T[indices])
legend([C,D,E], [ l1,l2 , l3], loc=2)
show()

[A,B,C ] = plot(Scores[3:].T[indices])
