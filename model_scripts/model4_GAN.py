...
theta_G = [v for v in tf.trainable_variables() ]
...
l2_losses_D = tf.add_n([tf.nn.l2_loss(v) for v in theta_D if 'bias' not in v.name]) * l2_lambda
l2_losses_G = tf.add_n([tf.nn.l2_loss(v) for v in theta_G if 'bias' not in v.name]) * l2_lambda
...
loss_val1 = G_loss + l2_losses_G
loss_val2 = D_loss + l2_losses_D


opti1 =tf.train.AdamOptimizer(1e-4 , beta1=0.5)
opti2 = tf.train.AdamOptimizer(1e-4 , beta1=0.5) 
train_op1 = tf.contrib.layers.optimize_loss(loss_val1, global_step=global_step,learning_rate=learning_rate, optimizer=opti1,clip_gradients=float(clip_gradients) , variables= theta_G)
train_op2 = tf.contrib.layers.optimize_loss(loss_val2, global_step=global_step,learning_rate=learning_rate, optimizer=opti2,clip_gradients=float(clip_gradients) , variables= theta_D)
...
for epoch in range(curr_epoch,num_epochs):
        ...
        for start, end in zip(range(0, number_of_training_data, batch_size),range(batch_size, number_of_training_data, batch_size)):
            ...
            curr_loss1,lr,_=sess.run([loss_val1,learning_rate,train_op1],feed_dict1)
            curr_loss1,lr,_=sess.run([loss_val1,learning_rate,train_op1],feed_dict1)
            curr_loss2,lr,_=sess.run([loss_val1,learning_rate,train_op1],feed_dict2)
            curr_loss2,lr,_=sess.run([loss_val1,learning_rate,train_op1],feed_dict2)
            curr_loss3,lr,_=sess.run([loss_val2,learning_rate,train_op2],feed_dict1)
            curr_loss4,lr,_=sess.run([loss_val2,learning_rate,train_op2],feed_dict2)
            loss,counter=loss+curr_loss1+curr_loss2,counter+1
            if counter %50==0:
                print("Epoch %d\t , Loss_G =%f \tLoss_D =%f" %(epoch , curr_loss1+curr_loss2 , curr_loss3+curr_loss4))
        sess.run(epoch_increment)
...
saver.save(sess, "model_GAN/model.ckpt")



# saver = tf.train.Saver()
# saver.restore(sess, "model_GAN/model.ckpt")
...
l1 = "golden_without_sigmoid"
l2 = "garbage_without_sigmoid"
l3 = "golden_with_sigmoid"
l4 = "garbage_with_sigmoid"
l5 = "golden_Rouge_L"
l6 = "garbage_Rouge_L"
l7 = "golden_Rouge_1"
l8 = "garbage_Rouge_1"
...
Scores = np.load("Scores_cnn")
[A,B,C,D,E,F,G,H,I,J] = plot(Scores.T)
legend([A,B,C,D,E,F,G,H,I,J], [ l1,l2,l3,l4,l5,l6,l7,l8,l9,l10], loc=2)
show()

indices = np.arange(50)
np.random.shuffle(indices)
[A,B,C,D,E,F,G,H,I,J] = plot(Scores.T[indices])
legend([A,B,C,D,E,F,G,H,I,J], [ l1,l2,l3,l4,l5,l6,l7,l8,l9,l10], loc=2)
show()
...
np.random.shuffle(indices)
[A,B,C,D,E,F,G,H,I,J] = plot(Scores[2:].T[indices])
legend([A,B,C,D,E,F,G,H], [l3,l4,l5,l6,l7,l8,l9,l10], loc=2)
show()
...
Scores = np.load("Scores_cnn0.npy")
indices = np.arange(50)
np.random.shuffle(indices)
