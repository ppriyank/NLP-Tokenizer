...
with tf.device('/gpu:1'):
    ...
    Embedding1 = tf.get_variable("Embedding1",initializer=w2v_embeddings) 
    Embedding2 = tf.get_variable("Embedding2",initializer=w2v_embeddings) 
    embedded_words6 = tf.nn.embedding_lookup(Embedding2,input_x3)
    embedded_words4 = tf.nn.embedding_lookup(Embedding2,input_x1)
    embedded_words5 = tf.nn.embedding_lookup(Embedding2,input_x2)
    embedded_words3 = tf.nn.embedding_lookup(Embedding1,input_x3)
    embedded_words1 = tf.nn.embedding_lookup(Embedding1,input_x1)
    embedded_words2 = tf.nn.embedding_lookup(Embedding1,input_x2)
    sentence_embeddings_expanded6=tf.expand_dims(embedded_words6,-1   )
    sentence_embeddings_expanded4=tf.expand_dims(embedded_words4,-1   )
    sentence_embeddings_expanded5=tf.expand_dims(embedded_words5,-1   )
    ...
    conv1=tf.nn.conv2d(sentence_embeddings_expanded4, filter1, strides=[1,1,1,1], padding="VALID", name="read_conv1" )
    conv2=tf.nn.conv2d(sentence_embeddings_expanded5, filter1, strides=[1,1,1,1], padding="VALID", name="read_conv2" )
    conv3=tf.nn.conv2d(sentence_embeddings_expanded6, filter1, strides=[1,1,1,1], padding="VALID", name="read_conv3" )
...
theta = [v for v in tf.trainable_variables()  if 'read' not in v.name]

# Read_D_loss = -tf.reduce_mean(tf.log(read_real) + tf.log(1. - read_fake))
# Read_G_loss = -tf.reduce_mean(tf.log(read_fake))
...
G_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=D_logit_real, labels=topic_label1))
D_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=D_logit_fake, labels=topic_label2))

G_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=D_real, labels=topic_label1))
D_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=D_fake, labels=topic_label2))



G_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=D_logit_real, labels=topic_label1))
D_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=D_logit_fake, labels=topic_label2))


G_loss = tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(logits=D_logit_real, labels=topic_label1))
D_loss = tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(logits=D_logit_fake, labels=topic_label2))
...
G_loss = tf.reduce_mean(tf.nn.l2_loss((D_real - topic_label1)))
D_loss = tf.reduce_mean(tf.nn.l2_loss((D_fake - topic_label2)))
...
# ***************************************
# loss_val1 = 100*(G_loss + D_loss) + L2_reg 
# loss_val2 =  L2_reg + 100* tf.reduce_sum(L_read) 

# opti1 =tf.train.AdamOptimizer(1e-4 , beta1=0.5)
# opti2 = tf.train.AdamOptimizer(1e-4 , beta1=0.5) 

# train_op1 = tf.contrib.layers.optimize_loss(loss_val2, global_step=global_step,learning_rate=learning_rate, optimizer=opti1,clip_gradients=float(clip_gradients) , variables= theta_read)
# train_op2 = tf.contrib.layers.optimize_loss(loss_val1, global_step=global_step,learning_rate=learning_rate, optimizer=opti2,clip_gradients=float(clip_gradients) , variables= theta)
...
[A,B,C ] = plot(Scores[4:7].T[indices])
legend([A,B,C], [ l6,l5 , l4 ], loc=2)
show()
