...
with tf.variable_scope('optimizers_loss'):
    ...
    loss_topic_total = topic_weight * loss_topic + tf.add_n([tf.nn.l2_loss(v) for v in theta_topic if 'bias' not in v.name]) * l2_lambda
    cond2 = tf.greater(tf.abs(output_z2 - D_real), 0.2)
    losses =  tf.where(cond2, 100 * tf.abs(output_z2 - D_real)  , tf.zeros_like(D_real) )
...
for epoch in range(0,num_epochs):
        ...
        for start, end in zip(range(0, number_of_training_data, batch_size),range(batch_size, number_of_training_data, batch_size)):
            ...
            feed_dict1 = {input_x2: trainX[start:end],dropout_keep_prob: 0.5 , input_x1: trainY[start:end] , input_x3: dummy_input1 ,\
                 output_z1 : trainZ[start:end , 0 ].reshape(batch_size,1)/4 , output_z2: trainZ[start:end , 1 ].reshape(batch_size,1)  }                 
            curr_loss3 , curr_loss2,_,_ =sess.run([loss_topic ,losses ,TOPIC,READ  ],feed_dict4)
