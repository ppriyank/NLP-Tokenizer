...
if model_select == 1 :
    ...
    saver.restore(sess, "final3/model.ckpt")    
elif model_select == 4: 
...
with tf.variable_scope('added_layer'):
    ...
    with tf.device('/gpu:%d'%(gpu)):
        ...
        W_class2 = tf.get_variable("W_class2",shape=[50, 1],initializer=initializer) #[embed_size,label_size]
...
with tf.variable_scope('optimizers_loss'):
    ...
    loss_read_total = tf.reduce_sum(L_read2) + L_read + tf.add_n([tf.nn.l2_loss(v) for v in theta_read if 'bias' not in v.name]) * l2_lambda
    delta1 = D_real - D_fake     
    if model_select == 1 or model_select == 2 :
        loss_topic = topic_weight * tf.reduce_sum(tf.log(1 + tf.exp(-1 * 10  * delta1))) 
    elif model_select == 4 or model_select == 5 or model_select == 3:
        confirm =  0
        if confirm == 0 :
            condition1 = tf.greater(read_fake, 0.2)
            condition2 = tf.greater(D_real, 0.7)
    ...
    # train_op1 = tf.contrib.layers.optimize_loss(loss_read_total, global_step=epoch_step,learning_rate=l_rate / (2.6*2.6 * 2.6 * 2.6) , optimizer=optopic6,clip_gradients=float(clip_gradients)\
    #  , variables= theta_read , increment_global_step=False )
...
folder_name = "fine_tuned11/"
saver = tf.train.Saver()
saver.restore(sess, folder_name +"model.ckpt")

folder_name = "fine_tuned14/"
saver = tf.train.Saver()
saver.restore(sess, folder_name +"model.ckpt")

folder_name = "fine_tuned4/"
saver = tf.train.Saver()
saver.restore(sess, folder_name +"model.ckpt")

folder_name = "fine_tuned1/"
saver = tf.train.Saver()
saver.restore(sess, folder_name +"model.ckpt")
...
for epoch in range(0,num_epochs):
        ...
        for start, end in zip(range(0, number_of_training_data, batch_size),range(batch_size, number_of_training_data, batch_size)):
            ...
            feed_dict2 = {input_x2: trainX[start:end],dropout_keep_prob: 0.5 , input_x1: trainY[start:end] , input_x3: dummy_input2 ,\
                    output_z1 : trainZ[start:end , 0 ].reshape(batch_size,1) , output_z2: trainZ[start:end , 1 ].reshape(batch_size,1) }
            feed_dict3 = {input_x2: trainX[start:end],dropout_keep_prob: 0.5 , input_x1: dummy_input1 , input_x3: dummy_input2 ,\
                output_z1 : SCORE_ONE  , output_z2: SCORE_ZERO  }        
...
import os    
folder_name  = "fine_tuned1%d" %(model_select)
...
start_rate = 0.001
with tf.variable_scope('NEW_OPTI4'):
    epoch_step=tf.Variable(0,trainable=False,name="Epoch_Step")
    epoch_increment=tf.assign(epoch_step,tf.add(epoch_step,tf.constant(1)))
    l_rate = tf.train.exponential_decay(start_rate, epoch_step, decay_steps,decay_rate, staircase=True)
    decay_steps, decay_rate = decay_steps, decay_rate
    optopic6 = tf.train.AdamOptimizer(1e-4 , beta1=0.5) 
    l2_lambda=0.001 
    losses = tf.nn.sigmoid_cross_entropy_with_logits(labels = output_z, logits = L2_output)
    losses = 1000* tf.reduce_sum(losses)
    train_op_main = tf.contrib.layers.optimize_loss(losses, global_step=epoch_step,learning_rate=l_rate, \
        optimizer=optopic6,clip_gradients=float(clip_gradients),  variables= [Theta_final2,Theta_final1,Theta_projection,Theta_L,Theta_embedding]\
         , increment_global_step=False)

    
opt = tf.get_collection( tf.GraphKeys.GLOBAL_VARIABLES, scope="NEW_OPTI4" ) 
sess.run(tf.variables_initializer(opt))
